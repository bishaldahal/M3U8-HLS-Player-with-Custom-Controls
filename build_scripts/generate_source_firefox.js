...
const outputFile = path.join(outputDir, `source-code.Firefox.${formattedDate}.zip`);
...
const command = `"Z:\\Programs\\7-Zip\\7z" a -tzip -r "${outputFile}" . -x!node_modules -x!.vscode -x!original -x!package-lock.json -x!README.md -x!extension.zip -x!.git -x!.gitignore -x!build_scripts -x!build -x!.github -x!.gitattributes -x!LICENSE -x!.vscode`;
